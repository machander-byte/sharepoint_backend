...
| Frontend tests | Done | `npm run test` passed, 3 tests. |
| Frontend lint | Pending | No `npm run lint` script exists in `ZMS.WebUI/package.json`. |
...
| API health/version | Done | Local SQLite smoke startup passed; `/api/health` returned `Healthy`, schema `Ready`, and `/api/version` returned `ZMS.API`. |
...
| EF CLI database update | Pending | `dotnet ef` is not installed on the workstation PATH; production DB update was not run. Startup schema initialization was smoke-tested with SQLite. |
...
| Final demo verification | Pending | Automated build/test/API smoke passed. Full browser demo flow requires live Supabase reviewer credentials and configured backend secrets. |
